...
def impute(df, imputer):
	'''
	imputes all missing values with imputer

	Parameters
	----------
	df: pandas dataframe
	imputer: Imputer whith the same interface as the sklearn IterativeImputers

	Returns
	-------
	df_res: imputed dataframe
	'''  
    X_res = imputer.fit_transform(df)
